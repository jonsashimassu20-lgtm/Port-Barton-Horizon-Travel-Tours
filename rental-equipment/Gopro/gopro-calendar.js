const goProInput = document.getElementById('gopro');
...
function applyHighlights() {
    ...
    let duration = parseInt(goProInput.value) || 1;
...
goProInput.addEventListener('input', applyHighlights);

bookbtn.addEventListener('click', (event) => {
    event.preventDefault();
    const selectedDateStr = document.getElementById('selected-date').value;
    const durationDays = parseInt(goProInput.value) || 1;
    ...
    if (selectedDateStr === "") {
        ...
        const newBooking = {
            equipmentType: "GoPro Rental",
            equipment: `${durationDays} Days`,
            date: finalDateRange,
            price: finalComputedPrice,
            Image: '/rental-equipment/Gopro/Image/go-pro1.jpg'
        };
