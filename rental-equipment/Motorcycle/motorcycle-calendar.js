...
function applyHighlights() {
    ...
    let duration = parseInt(motorCycleInput.value) || 1;

    if (duration > 45) {
        if (cappedError) cappedError.classList.add('show');
        duration = 45;
        motorCycleInput.value = 45;
    } else if (duration <= 45) {
...
motorCycleInput.addEventListener('input', applyHighlights);

bookbtn.addEventListener('click', (event) => {
    event.preventDefault();
    const selectedDateStr = document.getElementById('selected-date').value;
    const durationDays = parseInt(motorCycleInput.value) || 1;
    ...
    if (selectedDateStr === "") {
        ...
        const newBooking = {
            equipmentType: "Motorcycle Rental",
            equipment: `${durationDays} Days`,
            date: finalDateRange,
            price: finalComputedPrice,
            Image: '/rental-equipment/Motorcycle/image/honda-beat4.png'
        };
