...
function applyHighlights() {
    ...
    let duration = parseInt(pickUpInput.value) || 1;

    if (duration > 45) {
        if (cappedError) cappedError.classList.add('show');
        duration = 45;
        pickUpInput.value = 45;
    } else if (duration <= 45) {
...
pickUpInput.addEventListener('input', applyHighlights);

bookbtn.addEventListener('click', (event) => {
    event.preventDefault();
    const selectedDateStr = document.getElementById('selected-date').value;
    const durationDays = parseInt(pickUpInput.value) || 1;
    ...
    if (selectedDateStr === "") {
        ...
        const newBooking = {
            equipmentType: "Hilux(pick up) Rental",
            equipment: `${durationDays} Days`,
            date: finalDateRange,
            price: finalComputedPrice,
            Image: '/rental-equipment/Pick up/Image/pickup1.png'
        };
