const viosInput = document.getElementById('vios');
...
function applyHighlights() {
    ...
    let duration = parseInt(viosInput.value) || 1;
...
viosInput.addEventListener('input', applyHighlights);

bookbtn.addEventListener('click', (event) => {
    event.preventDefault();
    const selectedDateStr = document.getElementById('selected-date').value;
    const durationDays = parseInt(viosInput.value) || 1;
    ...
    if (selectedDateStr === "") {
        ...
        const newBooking = {
            equipmentType: "Vios(sedan) Rental",
            equipment: `${durationDays} Days`,
            date: finalDateRange,
            price: finalComputedPrice,
            Image: '/rental-equipment/Sedan/image/vios5.png'
        };
