const suvInput = document.getElementById('suv');
...
function applyHighlights() {
    ...
    let duration = parseInt(suvInput.value) || 1;
...
suvInput.addEventListener('input', applyHighlights);

bookbtn.addEventListener('click', (event) => {
    event.preventDefault();
    const selectedDateStr = document.getElementById('selected-date').value;
    const durationDays = parseInt(suvInput.value) || 1;
    ...
    if (selectedDateStr === "") {
        ...
        const newBooking = {
            equipmentType: "Fortuner(suv) Rental",
            equipment: `${durationDays} Days`,
            date: finalDateRange,
            price: finalComputedPrice,
            Image: '/rental-equipment/suv/Image/suv1.png'
        };
